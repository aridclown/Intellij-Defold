...
inline fun <R, U : R, T : R> T.letIf(
    condition: Boolean,
    block: (T) -> U
): R = if (condition) block(this) else this

inline fun <R, U : R, T : R> T.letIfNot(
    condition: Boolean,
    block: (T) -> U
): R = letIf(!condition, block)

/**
 * Checks whether the map has no blank keys or values.
 */
fun Map<String, String>.hasNoBlanks(): Boolean = haveNoBlanks(*keys.toTypedArray(), *values.toTypedArray())
